...
SYSTEM_PROMPT = (
    ...
    "Use IMAGE 2 to judge whether forward is safe. If IMAGE 2 shows the center lane blocked, "
    "do NOT choose forward even if IMAGE 1 shows a side opening — choose a turn toward that "
    "opening instead.\n\n"
    ...
    "GO FORWARD.\n"
    "  • Center-bottom shows a wall, object, box, bag, chair leg, bottle, cable, or any 3D "
    "barrier in the rover's direct path → forward is BLOCKED. Turn toward whichever side has "
    "visible floor.\n"
    ...
    "  • No visible floor anywhere → commit to a 60-90° turn toward the side with more structure.\n\n"
    ...
    "visible floor instead. Walls visible on the sides of a narrow corridor are NORMAL and "
    ...
    "COMMIT to one turn direction and use the blocked-lane turn staircase: first 45°, then 90°, "
    "then 135°, then 180° when allowed by max_turn_deg. If you choose the same turn direction "
    "again on the next tick, advance to the next step. Do NOT alternate tiny left/right nudges.\n"
    ...
    "is the bottom-half of the current frame; if it matches learned_floor_rgb closely "
    "(bot_dist_to_floor small, e.g. < 25) the immediate path is normal floor — safe to go "
    "forward. If bot_dist_to_floor is large AND bot_dist_to_wall is small, the rover's "
    "forward zone shows wall, not floor — do NOT pick forward.\n"
...
def _build_user_prompt(
    ...
    color_line = (
        ...
        "Low bot_dist_to_floor = floor seen in front; high that value + low "
        "bot_dist_to_wall = wall seen where floor should be."
        if floor_rgb
    ...
    if path_summary:
        parts.append(f"Immediate path analysis: {path_summary}")
    parts.append(
